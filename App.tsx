...
const ProtectedRoute: React.FC<
  React.PropsWithChildren<{ user: User | DemoUser | null; loading: boolean }>
> = ({ children, user, loading }) => {
  if (loading)
    return (
      <div className="h-screen w-full flex items-center justify-center bg-rose-50 dark:bg-[#130b20]">
    ...
    <>
      <Routes>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-3 bg-white dark:bg-white/5 p-3 md:p-2 rounded-2xl shadow-sm border border-pink-100 dark:border-white/10">
                  <div className="relative group w-full md:w-auto md:flex-1">
                    ...
                    {/* RIGHT-ALIGNED ICON – FIXED POSITION */}
                    <Search
                      className="
                        absolute
                        right-4           /* perfect alignment */
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setTypeFilter(
                          typeFilter === "expense" ? null : "expense"
                        )
                      }
                      className={`
                        px-4 py-2 rounded-xl text-sm font-bold transition-all
                  ...
                  <button
                    onClick={() => {
                      ...
                      setEndDate("");
                      setSearchQuery("");
                      setTypeFilter(null); // ✅ Reset new filter too
                    }}
                ...
                {paginatedTransactions.length === 0 ? (
                  <div className="text-center py-20 bg-white/50 dark:bg-white/5 rounded-3xl border-2 border-dashed border-slate-200 dark:border-white/10 backdrop-blur-sm">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Try changing your filters or add a new entry!
                    </p>
                  </div>
                ) : (
                  paginatedTransactions.map((t, index) => (
                    <Card
                      key={t.id}
                      className="p-3 md:p-5 flex justify-between items-center group hover:scale-[1.01] hover:shadow-pink-100/50 dark:hover:shadow-none transition-all duration-300"
                      style={{ animationDelay: `${index * 50}ms` }}
                    >
                      <div className="flex items-center gap-3 md:gap-5 flex-1 min-w-0">
                        {/* Icon Container - Smaller on Mobile */}
                        <div
                          className={`w-10 h-10 md:w-14 md:h-14 rounded-2xl flex items-center justify-center font-bold text-lg md:text-xl shadow-inner flex-shrink-0 transition-transform group-hover:rotate-6 ${
                            t.type === "income"
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-bold text-sm md:text-lg text-slate-800 dark:text-white truncate font-display">
                            {t.category}
                          </p>
                      ...
                      <div className="flex items-center gap-2 md:gap-8 flex-shrink-0 pl-2">
                        <span
                          className={`font-black text-sm md:text-xl whitespace-nowrap font-display ${
                            t.type === "income"
                              ? "text-emerald-500"
        ...
        <Route
          ...
          element={
            <BudgetPlanner userId={user.uid} transactions={transactions} />
          }
        />
...
const Splash: React.FC = () => (
  <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-rose-50 dark:bg-[#130b20] transition-colors duration-500">
    ...
    <p className="text-slate-400 font-medium mt-2 animate-pulse">
      Loading your vibes...
    </p>
  </div>
);
    ...
    <HashRouter>
      <Routes>
        ...
        <Route
          ...
          element={
            !currentUser ? (
              <Auth isRegister onDemoLogin={handleDemoLogin} />
            ) : (
              <Navigate to={AppRoute.DASHBOARD} />
            )
          }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <Card className="w-full max-w-md p-8 relative shadow-2xl border-2 border-white/50 dark:border-white/10">
            ...
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-display font-black bg-clip-text text-transparent bg-gradient-to-r from-pink-600 to-purple-600">
